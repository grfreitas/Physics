...
for k in range(25):
    ...
    for i in range(steps):
        ...
        plt.title(f'Passos = {i} | Distância até origem (em quadras): {abs(pos_x)+abs(pos_y)}')
        plt.pause(0.001)

    origin = [0],[0]
    plt.title(f'Passos = {steps} | Distância até origem (em quadras): {abs(pos_x)+abs(pos_y)}')
    plt.quiver(*origin, pos_x, pos_y, 
                ...
                alpha=0.7)
    if (pos_x == 0) | (pos_y == 0):
        plt.scatter(0,0,color='red', s=50, marker='+')
    else: 
        plt.scatter(0,0,color='black', s=50, marker='+')
    plt.pause(0.05)
    time.sleep(2)
